...
while user_dicision != 'stop':
    ...
    user_dicision = (input('Введите число неотрицательное число, чтобы получить его значение в ряду Трибоначи: '))                    #Сообщение от пользователя
    ...
    if user_dicision == 'test':
        ...
        for i in range(1,31):
            ...
            print(f'Число {i} = {test_number} найдено за {round(time_of_work, 2)} секунд, что больше предыдущего на {round(delta_proc, 3)}%')
    ...
    elif type(user_dicision) == int:
        if int(user_dicision) >= 0:
            time_of_work = time.time() - t1

            print(f'\nВаше число: {tribonachy(int(user_dicision))} найдено за {time_of_work}')
